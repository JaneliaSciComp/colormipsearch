...
class LocalTiffDecoder {
    ...
    static final int HOST_COMPUTER = 316;
    ...
    static final int JPEG_TABLES = 347;
    ...
    static final int META_DATA_BYTE_COUNTS = 50838; // private tag registered with Adobe
    static final int META_DATA = 50839; // private tag registered with Adobe
    ...
    LocalTiffDecoder(InputStream in, String name) {
        directory = "";
        this.name = name;
        url = "";
    ...
    RandomAccessStream getRandomAccessStream()
    {
        return in;
    }
    ...
    FileInfo[] getTiffInfo() throws IOException {
        ...
        if (list.size()==0) {
            ...
            FileInfo[] info = list.toArray(new FileInfo[0]);
